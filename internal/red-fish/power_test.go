	...
	tests := []testCase{
		{
			...
			sendResponse: func() (*http.Response, error) {
				...
				body, err := json.Marshal(
					PowerResponse{
						Response: PowerResp{
							CurrentPowerConsumption: 63180,
							PeakPowerConsumption:    108680,
						},
					},
				)
			...
			verifyMetrics: func(metrics []bmcmonitoring.Metric) {
				for _, metric := range metrics {
					err := bmcmonitoring.VerifyPower(metric)
					require.NoError(t, err)
				}
			},
		},
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			transport.RoundTripFunc = func(r *http.Request) (*http.Response, error) {
				...
				checkAuth(t, r)
				require.Equal(t, "https://hostname/redfish/v1/hl/oam/0/Power", r.URL.String())
				return test.sendResponse()
			}

			ch := make(chan []bmcmonitoring.Metric)
			go func() {
				ch <- client.Power(context.Background(), log)
			}()
