package redfish

import (
	"bytes"
	"context"
	"encoding/json"
	"errors"
	"fmt"
	bmcmonitoring "habana_bmc_exporter/pkg/bmc-monitoring"
	"habana_bmc_exporter/pkg/logger"
	"io"
	"net/http"
	"testing"

	"github.com/stretchr/testify/require"
)

func TestSensorCurrent(t *testing.T) {
	log := logger.New().WithField("test", "true")

	type testCase struct {
		name         string
		sendResponse func() (*http.Response, error)
		expected     func() []bmcmonitoring.Metric

		// check that the metric are with the expected labels
		verifyMetrics func(metrics []bmcmonitoring.Metric)
	}

	tests := []testCase{
		{
			name: "Red fish returns valid metrics",
			sendResponse: func() (*http.Response, error) {
				// send a valid response

				body, err := json.Marshal(
					SensorCurrentResponse{
						Response: SensorCurrentResp{
							VIn54:          1146,
							P1VIn12:        2485,
							Stage154VIn:    750,
							Stage113P5VOut: 3000,
							Stage213P5VIn:  4940,
							Stage2CoreOut:  72000,
							Stage2HBMOut:   6250,
						},
					},
				)
				require.NoError(t, err)

				return &http.Response{
					StatusCode: http.StatusOK,
					Body:       io.NopCloser(bytes.NewReader(body)),
				}, nil
			},
			expected: func() []bmcmonitoring.Metric {
				var metrics []bmcmonitoring.Metric
				prefix := bmcmonitoring.PrefixSensorCurrent

				// VIn54
				metrics = append(metrics, bmcmonitoring.Metric{
					Hostname:    "hostname",
					Oam:         "0",
					MetricValue: 1146,
					MetricName:  fmt.Sprintf("%s_%s", prefix, bmcmonitoring.SensorCurrentVin54),
				})

				// P1VIn12

				metrics = append(metrics, bmcmonitoring.Metric{
					Hostname:    "hostname",
					Oam:         "0",
					MetricValue: 2485,
					MetricName:  fmt.Sprintf("%s_%s", prefix, bmcmonitoring.SensorCurrentP1Vin12),
				})

				// Stage154VIn
				metrics = append(metrics, bmcmonitoring.Metric{
					Hostname:    "hostname",
					Oam:         "0",
					MetricValue: 750,
					MetricName:  fmt.Sprintf("%s_%s", prefix, bmcmonitoring.SensorCurrentStage154Vin),
				})

				// Stage113P5VOut
				metrics = append(metrics, bmcmonitoring.Metric{
					Hostname:    "hostname",
					Oam:         "0",
					MetricValue: 3000,
					MetricName:  fmt.Sprintf("%s_%s", prefix, bmcmonitoring.SensorCurrentStage113P5VOut),
				})

				// Stage213P5VIn
				metrics = append(metrics, bmcmonitoring.Metric{
					Hostname:    "hostname",
					Oam:         "0",
					MetricValue: 4940,
					MetricName:  fmt.Sprintf("%s_%s", prefix, bmcmonitoring.SensorCurrentStage213P5Vin),
				})

				// Stage2CoreOut
				metrics = append(metrics, bmcmonitoring.Metric{
					Hostname:    "hostname",
					Oam:         "0",
					MetricValue: 72000,
					MetricName:  fmt.Sprintf("%s_%s", prefix, bmcmonitoring.SensorCurrentStage2CoreOut),
				})

				// Stage2HBMOut
				metrics = append(metrics, bmcmonitoring.Metric{
					Hostname:    "hostname",
					Oam:         "0",
					MetricValue: 6250,
					MetricName:  fmt.Sprintf("%s_%s", prefix, bmcmonitoring.SensorCurrentStage2HBMout),
				})

				return metrics
			},
			verifyMetrics: func(metrics []bmcmonitoring.Metric) {
				for _, metric := range metrics {
					err := bmcmonitoring.VerifySensorCurrent(metric)
					require.NoError(t, err)
				}
			},
		},
		{
			name: "Red fish returns error",
			sendResponse: func() (*http.Response, error) {
				return &http.Response{
					StatusCode: http.StatusInternalServerError,
				}, errors.New("connection error")
			},
			expected: func() []bmcmonitoring.Metric {
				var metrics []bmcmonitoring.Metric
				return metrics
			},
		},
	}
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {

			transport := &transport{}

			client := &Client{
				log: log,
				Client: &bmcmonitoring.Client{
					ClientOpts: bmcmonitoring.ClientOpts{
						Oams:     1,
						Hostname: "hostname",
					},
					Client: http.Client{
						Transport: transport,
					},
				},
			}

			// check request and send response
			transport.RoundTripFunc = func(r *http.Request) (*http.Response, error) {

				checkAuth(t, r)
				require.Equal(t, "https://hostname/redfish/v1/hl/oam/0/SensorsCurrent", r.URL.String())
				return test.sendResponse()
			}

			ch := make(chan []bmcmonitoring.Metric)
			go func() {
				ch <- client.SensorCurrent(context.Background(), log)
			}()

			// wait until we finish to check the request
			got := <-ch
			require.Equal(t, test.expected(), got)

			if test.verifyMetrics != nil {
				test.verifyMetrics(got)
			}
		})
	}
}
